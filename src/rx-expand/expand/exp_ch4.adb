...
package body Exp_Ch4 is
   ...
   begin
      ...
      --  JMA
      if Generate_Plc_Code or else CodePeer_Mode then
         return;
      end if;
   ...
   begin
      --  JMA
      --  In Plc mode we skip the expansion of mod, as we want the generated
      --  as similar as the source code
      
      if Generate_Plc_Code then
         return;
      end if;
   ...
   begin
      ...
      if Operand_Type = Target_Type then
         ...
         --  JMA ??? Must add if Plc_Generating
      elsif Same_Representation (Operand_Type, Target_Type) then
         if Assignment_OK (N) then
            Set_Assignment_OK (Operand);
         end if;
         goto done;
   ...
   begin
	 ...
	 return;
      ...
      if Present (Actions (N)) then
         ...
         --  JMA
         if Generate_Plc_Code or else Minimize_Expression_With_Actions then
