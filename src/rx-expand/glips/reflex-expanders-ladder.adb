...
package body Reflex.Expanders.Ladder is
   ...
   begin
      ...
      while Present (Stmt) loop
         if Nkind (Stmt) = N_Procedure_Call_Statement then
            ...
            Break_If_Statements_List
	      (This, Insert_Node, Stmts, New_Copy_Tree (Cond));
            return;
         end if;
   ...
   procedure Expand_Op_Xor_For_Ladder
     (This : access Reflex_Expander_Record;
      ...
      Or1     : Node_Id;
   begin
      --  Expand : A xor B To (A and not B) or (not A and B);
   ...
   begin
      ...
      if Nkind (Rhs) in N_Op_Boolean then
         ...
         if Nkind (Parent (N)) = N_Op_Or
           or else Nkind (Parent (N)) = N_Op_And 
	 then

            --  Expand : A and not (C And D) To A and not C and not D

            --  Expand : A and not (C > D) To A and C < D
            ...
            if Nkind (Parent (N)) = N_Assignment_Statement then
               ...
               --  Expand : A := not (A and B) To not A := A and B

               Negate_Expr (Rhs);
               Rewrite (N, Rhs);

               --  This flag allow us to use not coil in Ladder But it is not
	       --  use currently
               ...
               --  Expand : not (A and C And D) To not A and not C and not D

               --  Expand : not (A > B) To A < B;
