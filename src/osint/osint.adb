...
package body Osint is
   ...
   begin
      if Std_Prefix = null then
         ...
         if Std_Prefix.all /= "" then
            ...
            ---- JMA set_std_prefix (Std_Prefix.all, Std_Prefix'Length - 1);
	    null;
         end if;
      end if;
   ...
   begin
	 ...
	 Len := Name'Last;
	 Keylen := Name'First;
	 while Name (Keylen + 1) /= ASCII.NUL
	   and then not Is_Directory_Separator (Name (Keylen + 1)) loop
	    Keylen := Keylen + 1;
	 end loop;

	 declare
	    Key : String := Name ((Name'First+1)..Keylen);
	 begin
	    ...
	    if Getenv (Key) /= null then
	       return Getenv (Key).all & Name ((keylen+1)..Len);
	    else
	       return "/usr" & Name ((keylen+1)..Len);
	    end if;
	 end;
      end if;
   end Translate_Name;
   ...
   --  function Update_Path_Old (Path : String_Ptr) return String_Ptr is
   ...
   function Update_Path (Path : String_Ptr) return String_Ptr is
      ...
      --JMA
     function C_Update_Path (Path, Key : String) return String;
	...
	-- Prefix : String := Std_Prefix.all & ASCII.NUL;
	Len    : Natural;
	Result : String_Ptr;
     begin
      declare
	 Prefix : String := Executable_Prefix.all & ASCII.Nul; -- Std_Prefix.all & ASCII.NUL;
      begin

      Len := C_String_Length (Prefix'Address);

	if (Path (Path'First..Path'First+Len) = Prefix (Prefix'First..Len))
	  and (Path (Len) = ASCII.NUL
		 or else Is_Directory_Separator (Path (Len)))
	  and Key /= "" then

	 if Key (Key'First) /= '$' then
	    Result := new String'("@" & Key & Path (Len..Path'Last));
	 else
	    Result := new String'(Key & Path (Len..Path'Last));
	 end if;

	 return Translate_Name (Result.all);
     ...
     Out_String : String_Ptr;
   begin
     Out_String := new String'(C_Update_Path (Path.all, "GCC"));
     return Out_String;
  end Update_Path;
