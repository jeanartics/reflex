...
package body Ch5 is
   ...
   ---------------------------------------
   -- P_Sequence_Of_Reactive_Statements --
   ---------------------------------------
   
   function P_Sequence_Of_Reactive_Statements
     (SS_Flags : SS_Rec) return List_Id is
   ...
   end P_Sequence_Of_Reactive_Statements;
   ...
   ----------------------------------
   -- 5.8  Reactive Goto Statement --
   ----------------------------------

   --  GOTO_STATEMENT ::= goto state_NAME;
   ...
   function P_Reactive_Goto_Statement return Node_Id is
      Goto_Node : Node_Id;

   begin
      Goto_Node := New_Node (N_Goto_Statement, Token_Ptr);
      Scan; -- past GOTO (or TO)
      Set_State_Name (Goto_Node, P_Qualified_Simple_Name_Resync);
      No_Constraint;
      TF_Semicolon;
      return Goto_Node;
   end P_Reactive_Goto_Statement;
   ...
   function P_Pause_Statement (LHS : Node_Id) return Node_Id is
   ...
   function P_Wait_Statement (LHS : Node_Id) return Node_Id is
      
      Wait_Node : Node_Id;
   begin
      Wait_Node := New_Node (N_Reactive_Wait_Statement, Prev_Token_Ptr);
      Set_Expression (Wait_Node, P_Expression_No_Right_Paren);
      TF_Semicolon;
      return Wait_Node;
   end P_Wait_Statement;
   
   ---------------------------------------
   -- 5.4  Select Statement Alternative --
   ---------------------------------------
   ...
   --  The caller has checked that the initial token is WHEN 
   --  Error recovery: can raise Error_Resync

   function P_Select_Statement_Alternative return Node_Id is
      
      Select_Alt_Node : Node_Id;
   begin
      Select_Alt_Node := New_Node (N_Select_Statement_Alternative, Token_Ptr);
      T_When; -- past WHEN (or give error in OTHERS case)
      Set_Expression (Select_Alt_Node, P_Expression_No_Right_Paren);
      TF_Arrow;
      Set_Statements
	(Select_Alt_Node, P_Sequence_Of_Reactive_Statements (SS_Sreq_Whtm));
      return Select_Alt_Node;
   end P_Select_Statement_Alternative;
   ...
   begin
      ...
      loop
	 if First_Alt then
            Append (P_Select_Statement_Alternative, Alternatives_List);
         ...
         --  complain about the missing WHEN, and discard the junk statements.
           ...
           or else Start_Column > Scope.Table (Scope.Last).Ecol
         then
            Error_Msg_BC ("WHEN (select statement alternative) expected");
      ...
      if No (First_Non_Pragma (Alternatives_List)) then
         Error_Msg
            ("WHEN expected, must have at least one alternative in select",
             First_When_Loc);
         return Error;

      else
         Set_Alternatives (Select_Node, Alternatives_List);
         return Select_Node;
      end if;
   end P_Select_Statement;

   -------------------------------------
   -- 5.4  Fork Statement Alternative --
   -------------------------------------
   ...
   function P_Fork_Statement_Alternative return Node_Id is
      
      Fork_Alt_Node : Node_Id;
   begin
      Fork_Alt_Node := New_Node (N_Reactive_Fork_Alternative, Token_Ptr);
      Set_Statements
	(Fork_Alt_Node, P_Sequence_Of_Reactive_Statements (SS_Sreq_Whtm));
      return Fork_Alt_Node;
   end P_Fork_Statement_Alternative;
   ...
   function P_Fork_Statement return Node_Id is
      
      Fork_Node         : Node_Id;
      Alternatives_List : List_Id;
      First_When_Loc    : Source_Ptr;
      First_Alt         : Boolean := False;
   begin
	 ...
	 if First_Alt then
	    ...
	    exit;
         ...
         --  complain about the missing WHEN, and discard the junk statements.
           ...
           or else Start_Column > Scope.Table (Scope.Last).Ecol
         then
            Error_Msg_BC ("AND THEN (fork statement alternative) expected");
      ...
      if No (First_Non_Pragma (Alternatives_List)) then
         Error_Msg
            ("must have at least one alternative in fork",
             First_When_Loc);
         return Error;

      else
         Set_Alternatives (Fork_Node, Alternatives_List);
         return Fork_Node;
      end if;
   ...
   function P_Reactive_Loop_Statement return Node_Id is
   ...
   begin
      ...
      Loop_Node := New_Node (N_Loop_Statement, Token_Ptr);
      Set_Is_Reactive_Statement (Loop_Node);
      TF_Loop;
      
      Set_Statements (Loop_Node, P_Sequence_Of_Reactive_Statements (SS_Sreq));
      End_Statements (Loop_Node);
      return Loop_Node;
   end P_Reactive_Loop_Statement;
   ...
   function P_Reactive_Exit_Statement return Node_Id is
   ...
   --  Start of processing for P_Reactive_Exit_Statement

   begin
      Exit_Node := New_Node (N_Exit_Statement, Token_Ptr);
      Set_Is_Reactive_Statement (Exit_Node, True);
      Scan; -- past EXIT

      if Token = Tok_Identifier then
         Dummy_Node := P_Qualified_Simple_Name;
	 Error_Msg_N ("LOOP label not permited in a reactive loop", Exit_Node);
      end if;
