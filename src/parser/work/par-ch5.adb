...
package body Ch5 is
   ...
   begin
      Goto_Node := New_Node (N_Reactive_Goto_Statement, Token_Ptr);
      Scan; -- past GOTO (or TO)
   ...
   function P_Select_Statement return Node_Id is
      ...
      First_When_Loc    : Source_Ptr;
   begin
      Select_Node := New_Node (N_Select_Statement, Token_Ptr);
   ...
   begin
      Fork_Alt_Node := New_Node (N_Fork_Statement_Alternative, Token_Ptr);
      Set_Statements
   ...
   function P_Fork_Statement return Node_Id is
      ...
      First_When_Loc    : Source_Ptr;
   begin
      Fork_Node := New_Node (N_Fork_Statement, Token_Ptr);
      ...
      if No (First_Non_Pragma (Alternatives_List)) then
         Error_Msg
            ("AND THEN expected, must have at least one alternative in fork",
             First_When_Loc);
         return Error;
   ...
   begin
	 ...
	 Error_Msg_N ("LOOP label not permited in a reactive loop");
      end if;
