...
package body Glips.Gen.Ch3 is
   ...
   begin
	 ...
	 null; -- Generate_Private_Type_Declaration (This, Node);
	 
      elsif Nkind (Node) = N_Private_Extension_Declaration then
	 null; -- Generate_Private_Extension_Declaration (This, Node);
...
--        Constr     : Node_Id;
   ...
   begin
	 ...
	 Write_Indent_Str (Ob, "_Ignore : Integer");
	 Write_Char (Ob, ';');
	 Write_Eol (Ob);
   ...
   begin
	 ...
	 Indent_Begin (Ob);
	 Generate_Component_List (This, Record_Extension_Part (Type_Def));
	 Indent_End (Ob);
   ...
   procedure Generate_Enumeration_Type_Definition_As_Constants  
     (This : access Glips_Generator_Record;
      Node : Node_Id) is
   ...
   begin
	 ...
	 exit when not No (Lit);
	 Write_Str (Ob, ", ");
      end loop;
      Write_Indent_Str (Ob, ");");
   end Generate_Enumeration_Type_Definition_As_Constants;
   ...
   procedure Generate_Incomplete_Type_Declaration 
     (This : access Glips_Generator_Record;
      ...
      Def_Id : Node_Id;
   begin
      if Glips.Options.Reduce_All then
	 null;
	 
	 --  Generate a forward type declaration
   ...
   procedure Generate_Modular_Type_Definition 
     (This : access Glips_Generator_Record;
      ...
      Expr     : Node_Id;
   begin
      Put_Line ("Generate_Modular_Type_Definition Begin");
      if Glips.Options.Reduce_All then
	 ...
	 Put_Line ("    Expr is " & Nkind (Expr)'Img);
	 Generate_Node (This, Expr);
	 Write_Str (Ob, ";");
	 Write_Eol (Ob);
      end if;
      Put_Line ("Generate_Modular_Type_Definition End");
   ...
   begin
	 ...
	 Write_Indent_Str (Ob, "type ");
	 Write_Id (Ob, Def_Id);
	 if Present (Real_Range_Specification (Type_Def)) then
	    Write_Str (Ob, " is range ");
	    Generate_Node (This, Real_Range_Specification (Type_Def));
	 else
	    Write_Str (Ob, " renames Float");
	 end if;
	 Write_Str (Ob, ";");
	 Write_Eol (Ob);
      end if;
   ...
   begin
	 ...
	 Subtype_Indication_As_String (This, Typ);
         ...
         --  Generate_Type_Name (This, Base_Type (Entity (Node)));
         Generate_Type_Name (This, Entity (Node));
      end if;
	 ...
	 if Compile_Time_Known_Value (Expr) then
	    if Is_Boolean_Type (Etype (Expr)) then
		  ...
		  Write_Str (Ob, "False");
               ...
               -- Write_Ureal_Col_Check (Ob, Expr_Value_R (Expr));
	 ...
	 if Present (Access_To_Subprogram_Definition (Type_Def)) then
	    if Comes_From_Source (Node) then
		 ...
		 ("reflex does not supported access to subprogram ", Node);
	       ...
	       --  to generate it as glips handles them.
	    else
	       return;
	    end if;
	 end if;
      ...
      --  Skip deferred constant, it will be generated when we encountered
      --  the full declaration of the constant.
      
      if Constant_Present (Node) and then No (Expression (Node)) then
	 return;
      end if;
...
--        Constr     : Node_Id;
   ...
   begin
      if Glips.Options.Reduce_All then
	 --  Nothing to do as the real types are generated as 
	 --  float. The references to the types are replaced by float 
   ...
   begin
      ...
      --  types are generated as integer in Glips. 
   ...
   begin
      ...
      loop
         Dim_Type   := Etype (Dim);
         Low_Bound  := Type_Low_Bound  (Dim_Type);
         High_Bound := Type_High_Bound (Dim_Type);
	 
         Write_Integer_Type
           (Ob,
	    UI_To_Int (Esize (Dim_Type)),
            Signed => not Is_Modular_Integer_Type (Dim_Type));
	 
	 Write_Str (Ob, " range ");
	 Generate_Node (This, Low_Bound);
	 Write_Str (Ob, " .. ");
	 Generate_Node (This, High_Bound);
   ...
   begin
      ...
      --  For each literal emit an integer constant wihch value is the 
   ...
   begin
      if Glips.Options.Reduce_All then
	 --  Nothing to do as the signed integer types are generated as 
	 --  integer. The references to the types are replaced by integer_x 
	 --  where x is the number of bits needed to holds a value of the type
	 null;
      else
	 Def_Id := Defining_Identifier (Node);
	 Type_Def := Type_Definition (Node);
   ...
   procedure Generate_Subtype_Declaration 
     (This : access Glips_Generator_Record;
      ...
      Subtyp : Node_id;
   begin
      Put_Line ("Generate_Subtype_Declaration Begin ");
	 ...
	 Put_Line 
	   ("   Not N_Subtype_Indication Subtyp is " & Nkind (Subtyp)'Img);
	 pragma Assert (Nkind_In (Subtyp, N_Expanded_Name, N_Identifier));
	 
	 Write_Indent_Str (Ob, "type ");
	 Write_Id (Ob, Defining_Identifier (Node));
	 Write_Str (Ob, " renames ");
	 ...
	 Generate_Subtype_Mark (This, Subtyp);
      ...
      Put_Line ("Generate_Subtype_Declaration End ");
      
   exception
      when others =>
	 Put_Line ("Generate_Subtype_Declaration exception");
	 raise;
   ...
   procedure Generate_Subtype_Indication 
     (This : access Glips_Generator_Record;
      ...
      Index       : Node_Id;
   begin
      Put_Line ("Generate_Subtype_Indication Begin " & Nkind (Node)'Img);
      -------------------------------
	 ...
	 --  If the subtype is constraining an array type, generte 
	 --  a Glips Type Instance
	 
      else -- if Is_Array_Type (Typ) then
	 Generate_Subtype_Mark (This, Mark);
      end if;
      
      Put_Line (" Constr " & Nkind (Constr)'Img);
	 ...
	 Write_Str (Ob, ")");
      ...
      elsif Nkind (Constr) = N_Range_Constraint then
	 Write_Str (Ob, " range ");
	 Generate_Range_Constraint (This, Constr);
	 
      elsif Nkind (Constr) = N_Range then
	 Write_Str (Ob, " range ");
	 Generate_Range (This, Constr);
      else
	 Error_Msg_N ("bad index for subtype indication", Node);
	 Generate_Node (This, Constr);
      end if;
      
      Put_Line ("Generate_Subtype_Indication End");
   exception
      when others =>
	 Put_Line ("Generate_Subtype_Indication exception");
	 raise;
