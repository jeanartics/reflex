...
--  	       end if;
	 ...
	 --  representation clause use directly their value.
      ...
      then
	 if Glips.Options.Reduce_All then
	    Write_Uint (Ob, Enumeration_Rep (Entity (Node)));
	 else
	    Write_Id (Ob, Node);
	 end if;
   ...
   procedure Generate_Identifier
     (This : access Glips_Generator_Record;
      ...
      Orig_Te : Entity_Id;
   begin
      Put_Line ("Generate_Identifier Begin");      
      E  := Entity (Node);
Put_Line ("E " & Ekind (E)'Img);      
      Te := Get_Type_Full_View (Scope (E));
      
      if Ekind (E) = E_Enumeration_Literal then
	 if Present (Get_Enumeration_Literal_Constant (E)) then
Put_Line ("Has literal constant");      
	    Write_Id (Ob, Get_Enumeration_Literal_Constant (E));
	    
	 elsif Has_Or_Inherits_Enum_Rep_Clause (Etype (E)) then
Put_Line ("Has representation literal constant");      
	    Write_Uint (Ob, Enumeration_Rep (E));

	 else
	    Write_Id (Ob, E);
	 end if;
	 
      Put_Line ("Generate_Identifier (enum) End");      
	 return;
      end if;
	 ...
	 while Present (Te) and then Te /= Orig_Te loop
      ...
      Put_Line ("Generate_Identifier End");      
   ...
   begin
	...
	or else Typ = Universal_Fixed
      then
	 Write_Str (Ob, "Float");
	 
      elsif Typ = Standard_Short_Short_Integer 
	or else Typ = Standard_Integer_8
      then
	 Write_Str (Ob, "Byte");
      elsif Typ = Standard_Short_Integer 
	or else Typ = Standard_Integer_16
      then
	 Write_Str (Ob, "Int");
      elsif Typ = Standard_Integer 
	or else Typ = Universal_Integer
      then
	 Write_Str (Ob, "Int");
      elsif Typ = Standard_Long_Integer 
	or else Typ = Standard_Long_Long_Integer 
	or else Typ = Standard_Integer_32
	or else Typ = Standard_Integer_64
      then
	 Write_Str (Ob, "Dint");
	 
      elsif Typ = Standard_Unsigned
	or else Typ = Standard_Short_Short_Unsigned
	or else Typ = Standard_Short_Unsigned
      then 
	 Write_Str (Ob, "Word");

     elsif Typ = Standard_Long_Unsigned 
       or else Typ = Standard_Long_Long_Unsigned
       or else Typ = Standard_Unsigned_64
       then
	 Write_Str (Ob, "Dword");
