...
package Reflex.Rungs is
   ...
   package Rungs_Lists is
     new Ada.Containers.Doubly_Linked_Lists (Rung_Ptr);
   use Rungs_Lists;
   
   function New_Rung return Rung_Ptr;
   
   function Get_Matrix
     (This : access Rung_Record) return access Matrices.Matrix_Record;
   procedure Set_Matrix
     (This : access Rung_Record;
      M    : access Matrices.Matrix_Record);
...
private
   ...
   No_Rung_Record : constant Rung_Record :=
     (Comment    => No_Str_Id,
      Box        => null,
      Max_Width  => Max_Unity_Ladder_Horizontal,
      Max_Height => Max_Unity_Ladder_Vertical,
      Width      => 0,
      Height     => 0,
      Matrix     => null);
