...
package body Urealp is
   ...
   procedure UR_Write_Reflex
     (Real      : Ureal;
      Brackets  : Boolean := False;
      To_Buffer : Boolean := False);
   ...
   function UR_Image
     (Real     : Ureal;
      Brackets : Boolean := False) return String is
   begin
      UR_Write_Reflex
	(Real      => Real,
	 Brackets  => Brackets,
	 To_Buffer => True);
      ...
      if UR_Image_Length = 0 then
	 return "";
      else
	 return UR_Image_Buffer (UR_Image_Buffer'First .. UR_Image_Length);
      end if;
   end UR_Image;
   ...
   is
      ...
      procedure Out_Write_Char (C : Character);
      procedure Out_Write_Str  (S : String);
      procedure Out_Write_Int  (Val : int);
      procedure Out_UI_Write
	(U      : Uint;
	 Format : UI_Format := Decimal);
      ...
      procedure Out_Write_Char (C : Character) is
      begin
	 if To_Buffer then
	    UR_Image_Length := UR_Image_Length + 1;
	    UR_Image_Buffer (UR_Image_Length) := C;
	 else
	    Write_Char (C);
	 end if;
      end Out_Write_Char;
      ...
      procedure Out_Write_Str (S : String) is
      begin
	...
	(U      : Uint;
	 ...
	 if Digs_Output = 4 and then Base = Uint_16 then
         ...
         Out_Write_Char (H (UI_To_Int (Uabs rem Base)));
	 ...
	 Digs_Output := Digs_Output + 1;
      ...
      procedure Out_Write_Int (Val : int) is
      begin
	 if Val < 0 then
	    Out_Write_Char ('-');
	    Out_Write_Int (-Val);
	    ...
	    if Val > 9 then
	       Out_Write_Int (Val / 10);
	    end if;

	    Out_Write_Char (Character'Val ((Val mod 10) + Character'Pos ('0')));
	 end if;
      end Out_Write_Int;

      -- Start of procesing of UR_Write
   begin
      UR_Image_Length := 0;
      ...
      then
         if Val.Den = 1 then
            T := Val.Num * (10/2);
            Out_UI_Write (T / 10, Decimal);
            Out_Write_Char ('.');
            Out_UI_Write (T mod 10, Decimal);

         elsif Val.Den = 2 then
            T := Val.Num * (100/4);
            Out_UI_Write (T / 100, Decimal);
            Out_Write_Char ('.');
            Out_UI_Write (T mod 100 / 10, Decimal);
            ...
            T := Val.Num * (1000 / 8);
            Out_UI_Write (T / 1000, Decimal);
            Out_Write_Char ('.');
            Out_UI_Write (T mod 1000 / 100, Decimal);
            ...
            Out_UI_Write (Val.Num * (Uint_2 ** (-Val.Den)), Decimal);
            Out_Write_Str (".0");
         end if;
      ...
      then
         ...
         then
            Out_UI_Write (Val.Num * Val.Rbase**(-Val.Den), Decimal);
            Out_Write_Str (".0");
         ...
         --  correct place.
            ...
            Out_Write_Str ("16#0.");
            Out_Write_Str (UI_Image_Buffer (4 .. UI_Image_Length));
            ...
            Out_Write_Char ('E');
            Out_UI_Write (Int (UI_Image_Length) - Val.Den, Decimal);

         elsif Val.Den = 1 then
            Out_UI_Write (Val.Num / 10, Decimal);
            Out_Write_Char ('.');
            Out_UI_Write (Val.Num mod 10, Decimal);

         elsif Val.Den = 2 then
            Out_UI_Write (Val.Num / 100, Decimal);
            Out_Write_Char ('.');
            Out_UI_Write (Val.Num / 10 mod 10, Decimal);
            Out_UI_Write (Val.Num mod 10, Decimal);
         ...
         --  Else use decimal exponential format
            ...
            Out_Write_Char (UI_Image_Buffer (1));
            Out_Write_Char ('.');

            if UI_Image_Length = 1 then
               Out_Write_Char ('0');
            else
               Out_Write_Str (UI_Image_Buffer (2 .. UI_Image_Length));
            end if;

            Out_Write_Char ('E');
            Out_UI_Write (Int (UI_Image_Length - 1) - Val.Den, Decimal);
         end if;
      ...
      --  normal Ada literal style if the numerator is one.
         ...
         Out_Write_Int (Val.Rbase);
         Out_Write_Str ("#1.0#E");
         Out_UI_Write (-Val.Den);
      ...
      --  where the denominator is 1.
         ...
         Out_UI_Write (Val.Num / Val.Den, Decimal);
         Out_Write_Str (".0");
   ...
   function Ureal_To_String (Real : Ureal) return String is
      
      R    : constant Ureal_Entry := Normalize (Ureals.Table (Real));
      Den  : Float;
      Num  : Float;
      Val  : Float;
   begin
      Put_Line ("R.Num " & Ui_To_Int (R.Num)'Img);
      Put_Line ("R.Den " & Ui_To_Int (R.Den)'Img);
      Put_Line ("R.Rbase " & R.Rbase'Img); 
      Put_Line ("R.Negative " & R.Negative'Img);
      
      Den := Float (Ui_To_Int (R.Den));
      Num := Float (Ui_To_Int (R.Num));
      Val := Num / Den;
