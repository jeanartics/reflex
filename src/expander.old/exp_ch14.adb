...
package body Exp_Ch14 is
   ...
   --  A graph vertex represents a waiting statement list. The
   --  statements are all represented by a vertex in the list.
   --  The K parameter is to choice the building of a conditional
   --  graph or not. The difference between a conditional graph
   --  a graph is that the conditonal graph holds a expression
   ...
   --  a Rlist which the vetices list of the graph.
   ...
   --  The declaration of the entities asscoitaed with vertex is done in two passes.
   --  The first one creates all entities corresponding to the states thet are not
   --  anonymous, it is to said that have a name given by the user in the source.
   --  The second pass creates the entities for all others states. This mechanism is
   --  used because we want to enter in declarative scope the entities corresponding
   --  to the user defined name states, in order to avoid names clasches with the
   --  interpretor created names. The entities associated with the states are boolean
   --  variables used by the interpretor is update the new state of the chart. The
   --  chart is composed of two type of states transient states and waiting states.
   --  The chart abandones the execution when it reaches a waitibng states, or the transition
   --  associated to the waiting states is false. The transient states have no transition
   --  and are created to simplify the interpretor. The transient states are activated and
   --  desactived in the same cyle of exceution.
   ...
   begin
      ...
      Append_To
	(Stmts,
	 New_Node (N_Reactive_Pause_Statement, Sloc (Last (Stmts))));
	 ...
	 case Rkind (V) is
	    ...
	    when R_If_Vertex =>
		  ...
		  Pre_Declar_Graph_Vertex (Else_Graph (V), Globals, Locals);
	       ...
	       Put_Line ("     R_If_Vertex End");

	    when R_Loop_Vertex =>
	       Put_Line ("     R_Loop_Vertex Begin");
	       Pre_Declar_Graph_Vertex (Body_Graph (V), Globals, Locals);
	       Put_Line ("     R_Loop_Vertex End");

	    when R_Exit_Vertex =>
	       null;

	    when R_Fork_Vertex =>
	       Put_Line ("     R_Fork_Vertex Begin");
	       Alt := First (Alternatives (V));
	       while Alt /= No_Rnode loop
		  Pre_Declar_Graph_Vertex (Alt, Globals, Locals);
		  Next (Alt);
	    ...
	    when R_Abort_Vertex
	      | R_Weak_Abort_Vertex =>
	       Put_Line ("     R_Abort_Vertex Begin");
	       Pre_Declar_Graph_Vertex (Body_Graph (V), Globals, Locals);

	       if Handler_Graph (V) /= No_Rnode then
		  Pre_Declar_Graph_Vertex (Handler_Graph (V), Globals, Locals);
	       end if;
	       Put_Line ("     R_Abort_Vertex End");

	    when R_Pause_Vertex =>
	       Put_Line ("     R_Pause_Vertex Begin");
	       if Present (Step_Identifier (Item_Node (V))) then
		  Declar_Waiting_Vertex (V, Globals, Locals);
	       end if;
	       Put_Line ("     R_Pause_Vertex End");

	    when R_Wait_Vertex =>
	       Put_Line ("     R_Wait_Vertex Begin");
	       if Present (Step_Identifier (Item_Node (V))) then
		  Put_Line ("     Step Identifier is present");
		  Declar_Waiting_Vertex (V, Globals, Locals);
	       end if;
	       Put_Line ("     R_Wait_Vertex End");

	    when R_Sync_Vertex =>
	       Put_Line ("     R_Sync_Vertex Begin");
		  ...
		  Declar_Waiting_Vertex (V, Globals, Locals);
	       ...
	       Put_Line ("     R_Sync_Vertex End");

	    when R_Select_Vertex =>
	       Put_Line ("     R_Select_Vertex Begin");
	       if Present (Step_Identifier (Item_Node (V))) then
   ...
   begin
	 ...
	 case Rkind (V) is
	    ...
	    when R_Graph_Vertex =>
	       Put_Line (" R_Graph_Vertex Begin");
	       Declar_Graph_Vertex (V, Globals, Locals);
	       Put_Line (" R_Graph_Vertex End");

	    when R_Contionnal_Graph_Vertex =>
	       Put_Line (" R_Conditional_Graph_Vertex Begin");
	       Declar_Graph_Vertex (V, Globals, Locals);
	       Put_Line (" R_Conditional_Graph_Vertex End");
	    ...
	    when R_Exit_Vertex =>
	       Put_Line (" R_Exit_Vertex Begin");
	       Declar_Exit_Vertex (V, Locals);
	       Put_Line (" R_Exit_Vertex End");
	    ...
	    when R_Sync_Vertex =>
	       Put_Line (" R_Wait_Sync Begin");
	       Declar_Waiting_Vertex (V, Globals, Locals);
	       Put_Line (" R_Wait_Sync End");

	    when R_Select_Vertex =>
	       Put_Line (" R_Select Begin");
	       Declar_Select_Vertex (V, Globals, Locals);
   ...
   begin
	...
	(Loc,
	 ...
	 Xter_Id   := Make_Defining_Identifier (Sloc  => Loc, Chars => Xter_Name);
	 ...
	 Xinimac_Id   := Make_Defining_Identifier (Sloc  => Loc, Chars => Xinimac_Name);
...
--        Set_Initialize_Code (Gn, New_List (If_Stmt));
   ...
   begin
      ...
      Set_Then_Graph
	(Rn, Make_Graph_Vertex (Empty, Then_Statements (N)));
	...
	(Rn,
	 ...
	 Set_Alternatives (Rn, New_List);
      ...
      if Present (Else_Statements (N)) then
	 Set_Else_Graph
	   (Rn, Make_Graph_Vertex (Empty, Else_Statements (N)));
      end if;
