...
package body Exp_Ch14 is
   ...
   --  A graph vertex represents a reactive statement list. Each statement is
   --  represented by a vertex in the list. The K parameter is to choice
   --  the building of a conditional graph or not. The difference between a
   --  conditional graph is that the conditonal graph holds a expression
   --  that condition the graph.

   function Analyze_Statements
     (Gn : Rnode_Id;
      L  : List_Id) return RList_Id;
   --  Used to walk throught the statement list, and build the corresponding
   --  vertices list of control statement, reactive and if, case and loop
   --  construct. The vertices are linked in sort that a vertex has only one
   --  input and only one output. The list is a single direct graph. 
   ...
   begin
      ...
      Append_To (Stmts, Pause_Vertex_Class_Ptr (Pause_Vertex));
...
--        Set_Initialize_Code (Gn, New_List (If_Stmt));
   ...
   begin
      Graph_Vertex := New_Graph_Vertex (N);

      --  Create Begin and End Vertex of the graph

      Set_Begin_Vertex (Graph_Vertex, New_Begin_Vertex (N));
      Set_End_Vertex (Graph_Vertex, New_End_Vertex (N));
	 ...
	 Set_Vertices_List
	   (Graph_Vertex, Analyze_Statements (Graph_Vertex, L));
      end if;
   ...
   function Analyze_Statements
     (Graph : Rnode_Id;
      Stmts : List_Id) return Vertices_Lists.List is

      Vertices        : Vertices_Lists.List;
      Current_Vertex  : Vertex_Id := No_Rnode;
	 ...
	 if Previous_Vertex = No_Vertex then
	    Append_Enter_Code (Graph, Code);
	 else
	    Append_Exit_Code (Previous_Vertex, Code);
	 end if;
	 
	 Code := No_List;
	 Previous_Vertex := Current_Vertex;
	 
	 Append_To (Vertices, Current_Vertex);
   ...
   begin
	 ...
	 case Nkind (Stmt) is
      ...
      if Present (Current_Vertex) then
	 Append_Exit_Code_To_Previous_Vertex (Current_Vertex, Code);
      else
	 Append_Exit_Code_To_Previous_Vertex (Gn, Code);
      end if;
   ...
   function Make_Pause_Vertex (N : Node_Id) return Vertex_Id is
      V : Vertex_Id;
   begin
      V := New_Pause_Vertex (N);
      return Rn;
   ...
   begin
      ...
      V := New_Wait_Vertex (N);
      Set_Condition (V, New_Copy_Tree (Condition (N)));
   ...
   function Make_Select_Vertex (N : Node_Id) return Rnode_Id is
      ...
      --  Create the graph for a select alternative. Create a 
      --  R_Conditional_Graph
      ...
      function Make_Select_Alternative (Alt_Node : Node_Id) return Rnode_id is
	 Alter_Graph : Rnode_Id;
      begin
	 Aletr_Graph := Make_Graph_Vertex
	   (Alt_Node, Statements (Alt_Node), R_Contionnal_Graph_Vertex);

	 Set_Condition
	   (Ag,
	    New_Copy_Tree (Condition (Alt_Node)));
	 return Ag;
      end Make_Select_Alternative;

   begin
      Vselect := New_Select_Vertex (N);
      Rn := New_Node (R_Select_Vertex);
      New_Nodes_Association (N, Rn);

      -- Create the end select vertex.

      Vend := New_End_Vertex (N);
			      
      Set_End_Vertex (Vselect, Vend);
	 ...
	 Set_Alternatives (Rn, New_List);
         ...
         Alt := First (Alternatives (N));
         while Present (Alt) loop
            Append_To (Alternatives (Rn), Make_Select_Alternative (Alt));
	    Next (Alt);
         end loop;
      end if;
   ...
   function Make_If_Vertex (N : Node_Id) return Rnode_Id is
	 ...
	 Set_Condition (Ag, New_Copy_Tree (Condition (Anode)));
	 return Ag;
      end Make_If_Alternative;

   begin
      If_Vertex := New_If_Vertex (N);
      
      --  Create the associated transient scope
      --  Make_State (Is_Transient => True);
      
      -- Create the end select vertex.

      Vend := New_End_Vertex (N);
      Set_End_Vertex (If_Vertex, Vend);

      Set_Then_Graph
	(If_Vertex, Make_Graph_Vertex (Empty, Then_Statements (N)));

      Set_Condition (If_Vertex, New_Copy_Tree (Condition (N)));

      -- Build graf of all alternatives.

      if Present (Elsif_Parts (N)) then
	 Set_Alternatives (If_Vertex, New_List);

         E := First (Elsif_Parts (N));
         while Present (E) loop
            Append_To (Alternatives (Rn), Make_If_Alternative (E));
            Next (E);
         end loop;
      end if;

      if Present (Else_Statements (N)) then
	 Set_Else_Graph (Rn, Make_Graph_Vertex (Empty, Else_Statements (N)));
      end if;
...
--        end if;
   ...
   procedure Make_Activation_Desactivation
     (Stmts     : List_Id;
      V_Current : Rnode_Id;
      V_Next    : Rnode_Id) is
   ...
   end Make_Activation_Desactivation;
...
--     end Need_To_Prefixed;
   ...
   procedure Reset_Name_Statements (N : Node_Id) is
      ...
      function Process_Reactive_Statement_Name
   ...
   procedure Make_Reactive_Type_Declaration
     (T     : Node_Id;
      ...
      Pack_Spec : Entity_Id := Enclosing_Package (T);
      Visible_Decls : List_Id;
      Private_Decls : List_Id;
      
      Type_Decl : Node_Id := Parent (T);
      --  Reactive type declaration
      
      Type_Def  : Node_Id := Type_Definition (Type_Decl);
      
      Pack_Node : Node_Id;
      
      Literals : List_Id;
      Lit : Node_Id;
      --  Literals of states type enumeration
      
      Enum_Type : Node_Id;
      Emun_Def : Node_Id;
      --  The enumeration type of states 
      
      State : Node_Id;
      --  State of reactive type T
      
      Array_Type_Def : Node_Id;
      Array_Type_Id : Node_Id;
      Array_Full_Type : Node_Id;
      --  Declartion of boolean array declartion, one boolean per state
      
      Items : List_Id;
      Comps : Node_Id;
      --  Component list node, containing all infos to interprter of graph
      
      New_Type_Decls : Node_Id;
      --  The full type declaration replacing the reactive type T
      
      Xc : Node_Id;
      --  Boolean array of states for current actif states
      
      Xa : Node_Id;
      --  Boolean array of states for current states to actvate
      
      Xd : Node_ID;
      --  Boolean array of states for current states to desactvate
      
      Xp : Node_Id;
      --  Boolean array of states for previous actif states 
      
      Xr : Node_Id;
      --  Boolean array of states for raising states, so states just activate
      
      Xf : Node_Id;
      --  Boolean array of states for falling states, so states just desactivate
      
   begin
      --  Make the states enumeration type definition. Create a literal for
      --  each state
      
      Literals := New_List;
      Enum_Def := Make_Enumeration_Type_Definition
	(Sloc       => Sloc (T),
	 Literals   => Literals);
	 ...
	 S : String := Get_String (Chars (T)) & "_" & "state_type");
      begin
	 Enum_Type := Make_Full_Type_Declaration
	   (Sloc                => Sloc (T),
	    Defining_Identifier => 
      ...
      State := First (States (Type_Decl));
      while Present (State) loop
	 Lit := Make_Defining_Identifier
	   (Chars (Defining_Identifier (State)), Sloc (T));
	 Append (Lit, Literals);
	 Next (State);
      end loop;
      
      --  Make array of states type definition with index of type the 
      --  enumeration type just defined :
      --   type T_State_Array_Type is array (Literals'First..Literals'Last) 
      --          of Boolean;
	...
	(Make_Range_Constraint 
	   (Sloc             => Sloc (T),
	    Range_Expression =>
		 ...
		 (Sloc       => Sloc (T),
		  Low_Bound  => 
		    New_Reference_To (First (Literals), Sloc (T)),
		  High_Bound => 
		    New_Occurrence_Of (Last (Literals), Sloc (T)))),
	    Component_Definition => New_Occurrence_Of (Standard_Boolean, Loc));
	 ...
	 Array_Full_Type := Make_Full_Type_Declaration
      ...
      --  Declare the corresponding derived type. First declare the record
      --  extension. The resulting type is
      --   type T is tagged record
      --      Init : Boolean;
      --      Xc : T_State_Array_Type := (others => False);
      --      Xa : T_State_Array_Type := (others => False);
      --      Xd : T_State_Array_Type := (others => False);
      --      Xp : T_State_Array_Type := (others => False);
      --      Xr : T_State_Array_Type := (others => False);
      --      Xf : T_State_Array_Type := (others => False);
      --      Locals : T_Local_Record;
      --   end record;
	...
	(Sloc             => Sloc (T),
	 Component_Items  => Items);
      ...
      --  Initialization to be donne
      
      --  Current States
      
      Xc_Array := Make_Component_Declaration
	(Sloc                  => Sloc (T),
	 Defining_Identifier   =>
	   Make_Defining_Identifier 
	   (Sloc => Sloc (T),
	    Chars => String_Find ("xc")),
	 Component_Definition  => New_Occurence_Of (Array_Type_Id, Sloc (T)),
	 Expression            => Empty);
      --  (others => False)
      
      Append (Xc_Array, Items);
	
      --  States to activate
      
      Xa_Array := Make_Component_Declaration
	(Sloc                  => Sloc (T),
	 Defining_Identifier   =>
	   Make_Defining_Identifier 
	   (Sloc => Sloc (T),
	 ...
	 Expression            => Empty);
      ...
      Append (Xa_Array, Items);
      
      --  States to desactivate
      
      Xd_Array := Make_Component_Declaration
	(Sloc                  => Sloc (T),
	 Defining_Identifier   =>
	   Make_Defining_Identifier 
	   (Sloc => Sloc (T),
	    Chars => String_Find ("xd")),
	 Component_Definition  => New_Occurence_Of (Array_Type_Id, Sloc (T)),
	 Expression            => Empty);
      --  (others => False)
      
      Append (Xd_Array, Items);
      
      --  Previous States
      
      Xp_Array := Make_Component_Declaration
	(Sloc                  => Sloc (T),
	 Defining_Identifier   =>
	   Make_Defining_Identifier 
	   (Sloc => Sloc (T),
	    Chars => String_Find ("xp")),
	 Component_Definition  => New_Occurence_Of (Array_Type_Id, Sloc (T)),
	 Expression            => Empty);
      --  (others => False)
      
      Append (Xp_Array, Items);
	
      --  Raising States
      
      Xr_Array := Make_Component_Declaration
	(Sloc                  => Sloc (T),
	 Defining_Identifier   =>
	   Make_Defining_Identifier 
	   (Sloc => Sloc (T),
	    Chars => String_Find ("xr")),
	 Component_Definition  => New_Occurence_Of (Array_Type_Id, Sloc (T)),
	 Expression            => Empty);
      --  (others => False)
      
      Append (Xr_Array, Items);
	
      --  Falling States
      
      Xf_Array := Make_Component_Declaration
	(Sloc                  => Sloc (T),
	 Defining_Identifier   =>
	   Make_Defining_Identifier 
	   (Sloc => Sloc (T),
	    Chars => String_Find ("xf")),
	 Component_Definition  => New_Occurence_Of (Array_Type_Id, Sloc (T)),
	 Expression            => Empty);
      --  (others => False)
      
      Append (Xf_Array, Items);
      
      --  Full type de claration, type definition is a Record
	...
	(Sloc            => Sloc (T),
	 ...
	 Component_List  => Comps);
      ...
      New_Type_Decls := Make_Full_Type_Declaration
	(Sloc                       => Sloc (T),
	 Defining_Identifier        => 
	   Make_Defining_Identifier
	      (Sloc => Sloc (T),
