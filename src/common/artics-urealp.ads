...
package Artics.Urealp is
   ...
   UR_Image_Max    : constant := 64;
   UR_Image_Buffer : String (1 .. UR_Image_Max);
   UR_Image_Length : Natural;
   --  Buffer used for UR_Image as described below

   function UR_Image
     (Real     : Ureal;
      Brackets : Boolean := False) return String;
   --  Places a representation of Ureal, consisting of a possible minus sign,
   --  followed by the value in UR_Image_Buffer. The value must fit in
   --  UR_Image_Buffer. If necessary, the result is an approximation of the
   --  proper value, using an exponential format. The image of No_Ureal is 
   --  output as a single question mark.
